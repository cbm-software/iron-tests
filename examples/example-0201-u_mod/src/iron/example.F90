...
! Main differences with Andreas' version:
! - load increments: 2 vs. 1
! - added deformed field for visual.
! - max. number iterations set to 100
! - Neumann point vs. Neumann integrated
! - Scaling!?!?!?
  ...
  REAL(CMISSRP) :: BCDISP_MAX,NeumannBCvalue,lambda,areaChange,GaussXi,NodalForce, appliedForce
  ...
  INTEGER(CMISSIntg) :: NumberOfLoadIncrements=2 ! WAS 1
...
! Get only bc
  IF(NUMBER_OF_ARGUMENTS == 1) THEN
    ...
    IF((bcDirichlet<0).OR.(bcDirichlet>2)) CALL HANDLE_ERROR("Invalid BC type.")
  ELSE
    bcDirichlet = 1 ! 0: Dirichlet 1:Neumann int 2:Neumann point
  END IF
! Get all
  ...
  ELSE
    ...
    ! moved up
    !bcDirichlet                   = 2 ! 0: Dirichlet 1:Neumann int 2:Neumann point
  ENDIF
  ...
  CASE(3)
    appliedForce = 2.0_CMISSRP * MooneyRivlin1 * (lambda - 1.0_CMISSRP / lambda / lambda)
  CASE(2)
    appliedForce = 2.0_CMISSRP * MooneyRivlin1 * (lambda - 1.0_CMISSRP / (lambda * lambda * lambda))
  CASE DEFAULT
    CALL HANDLE_ERROR("Invalid number of dimensions!!")
  END SELECT

  WRITE(*,*) "Applied Force is "
  WRITE(*,*) appliedForce
...
!  CALL cmfe_Equations_OutputTypeSet(equations,CMFE_EQUATIONS_MATRIX_OUTPUT,err) 
  ...
  IF(useGeneratedMesh==1) THEN
    ...
    CALL cmfe_Decomposition_NodeDomainGet(Decomposition,NodeNumber,1,NodeDomain,Err)
    IF(NodeDomain==ComputationalNodeNumber) THEN
      DO component_idx=2,NumberOfDimensions !,3
        CALL cmfe_BoundaryConditions_AddNode(BoundaryConditions,DependentField,CMFE_FIELD_U_VARIABLE_TYPE,1,1,NodeNumber, &
    ...
    CASE(1)
      ...
      ! Weight with surface for integration (mesh reader) 
      IF (numberOfDimensions==3) THEN
        NeumannBCvalue = appliedForce * HEIGHT * LENGTH
      ELSE! IF (numberOfDimensions==2)
        NeumannBCvalue = appliedForce * HEIGHT
      END IF
      ...
      CALL GeneratedMesh_SurfaceWeightsGet(nodalWeights,CMFE_GENERATED_MESH_REGULAR_RIGHT_SURFACE, &
        & numberGlobalXelements,numberGlobalYelements,numberGlobalZelements,DisplacementInterpolationType,Err)
      ...
      IF(NodeDomain==ComputationalNodeNumber) THEN
        ...
        CASE (1)
          ...
          CALL cmfe_BoundaryConditions_AddNode(BoundaryConditions,DependentField, &
            ...
            & 1,CMFE_BOUNDARY_CONDITION_NEUMANN_INTEGRATED,NodalForce,Err)
        CASE (2)
          WRITE(*,*) "Applying traction WITH NEUMANN POINT.."
          ! corresponding traction value
          NeumannBCvalue = appliedForce
          ! apply point forces, Neumann BC at x=lx
          CALL HANDLE_ERROR("Error Neumann point for user-defined mesh!")
        CASE DEFAULT
          CALL HANDLE_ERROR("Error for BC!")
        END SELECT 
      END IF
